...
**对外 SSE 帧**仅允许三种写出：`event: snapshot` 与 `event: update` 的 data MUST 为与 REST `GET /api/v1/sessions/active` 同构的 `ActiveSessionItem` 裸数组；心跳 MUST 为注释行 `: ping`，无 data。MUST NOT 把内部 `Type` 名用作 SSE `event:`，MUST NOT 发送领域 Payload、增量 diff、单任务补丁或 error 事件帧。
...
- **THEN** 客户端只收到 `snapshot` 和/或 `update` 帧（若该 Type 被消费过滤表标脏），data 均为完整裸数组，帧的 `event` 名不出现内部 Type

#### Scenario: CreateTask 发领域事件但不驱动指挥中心 SSE

- **WHEN** `CreateTask` 插入可见行成功
- **THEN** 总线发布 `task.created`，已连接的指挥中心 SSE 不因此单独推送 `update`
...
- **WHEN** 客户端请求 `GET /api/v1/sessions/active`
