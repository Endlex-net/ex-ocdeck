...
数据源：指挥中心 MUST 从 App 层共享 projects 轮询 store（5s single-flight，与侧栏同一数据源，MUST NOT 自行重复轮询 `/projects`）获取全状态任务树，并以 `GET /api/v1/sessions/active/stream` 的 SSE 订阅快照（见 active-sessions-stream spec）补充活跃任务的 `last_active_at`、`agentStatus` 与 `attention`。
...
- **WHEN** 指挥中心已展示数据，随后收到 sessions/active 的 update 帧
- **THEN** 页面按 join 规则以新快照收敛对应分区，不发起对 sessions/active 的轮询请求

#### Scenario: 订阅异常保留旧数据

- **WHEN** sessions/active 的 SSE 订阅断开且重连尚未成功，或 projects 轮询失败
- **THEN** 页面保留上次成功数据并展示错误提示，不闪现空态

#### Scenario: projects 轮询不重叠与失败保留

- **WHEN** 指挥中心共享 projects store 的某次轮询超过 5 秒未返回，随后下一次轮询失败
