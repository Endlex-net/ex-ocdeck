...
- 命令面板四项配置（热键/触发词/匹配模式/指令触发词）后端持久化 + 设置页编辑，运行时生效。
...
多指令触发词最长前缀解析（4.9-4.11 前置契约）：触发词解析在「全局触发词（快速新建，余文为项目名）+ 已启用指令触发词（`commandTriggers` 非空值，余文忽略，`projects` 例外）」集合上按最长前缀匹配（fold 比较，`触发词 + 空白` 即进入模式）；同长度前缀不可能冲突（配置校验禁止重复值）；值之间前缀重叠允许（解析按最长前缀优先）；仅触发词无尾随空白不进入任何模式（保持既有模糊匹配）。规范源为 spec web-ui-shell「⌘K 全局命令面板 · 指令触发词模式」。projects 指令例外（4.12 增量，人工冒烟反馈）：`projects` 指令余文作为项目名（空余文视为空查询，展示全部项目候选）——复用快速新建模式的候选渲染/排序/缩写档位/零命中 fallback/默认高亮全套机制；项目候选与置顶「项目管理」命令的 Enter 导航 `#/projects#<projectID>` 深链（置顶命令按唯一精确/唯一子串推断目标项目，缩写不参与推断，无推断命中时导航 `/projects` 不选中）；机制规范源为 spec web-ui-shell「指令触发词模式 · projects 指令项目参数」。
...
```ts
type MatchKind = 'exact' | 'prefix' | 'substring' | 'acronym';
...
`rankByQuery` MUST NOT 修改输入数组，返回原 item 引用的新数组。exact/prefix/substring 命中时 `index = foldForMatch(name).indexOf(foldForMatch(query))`，因此 exact/prefix 恒为 `0`、substring 为首个命中位置的 UTF-16 code-unit 下标；acronym 命中的 `index` 语义见下方缩写档位段（固定为 `0`）；空查询或未命中返回 `null`（无 MatchResult）。

缩写档位（acronym，人工冒烟二期反馈新增）：新增 `acronymOf(name): string` 纯函数——对原始项目名按 `-`、`_`、空白字符（同一 ECMAScript WhiteSpace + LineTerminator 集合）拆段（跳过空段）；每段内再按 camelCase 边界拆子段，边界定义为「前一字符不是 ASCII 大写字母（`A-Z`）且当前字符是 ASCII 大写字母」；取每个非空子段的首字符，逐个经 `foldForMatch` 折叠后按序拼接（例：`go-ai-agent-app` 与 `goAiAgentApp` 的缩写均为 `gaaa`；数字段如 `app2` 取 `a`）。匹配规则：非空查询 `q` 且 `acronymOf(name)` 非空且 `acronymOf(name).startsWith(foldForMatch(q))` 时为 acronym 命中（`g`/`ga`/`gaa`/`gaaa` 命中、`aa` 不命中）。`acronymOf` MUST 在原始名称上分段（fold 会消灭 camelCase 边界信息，MUST NOT 先 fold 再分段）；camelCase 边界判定仅看 ASCII 大写，不使用 Unicode 大小写判定。acronym 命中的 `MatchResult.index` 固定为 `0`（不参与位置比较）。排序档位扩展为 exact > prefix > substring > acronym：acronym 档内无位置 tie-break，直接按名称确定序、再输入顺序兜底；同一项目同时命中 substring 与 acronym 时按更高档 substring 计。缩写档位不受 `matchMode` 影响，MUST NOT 参与 `matchMode` 的自动预选推断（预选推断仍仅 exact/唯一子串）；acronym 命中计入非空查询的命中集合（零命中 fallback 不触发、默认高亮移至首个候选的既有规则随之生效）。
...
- 领域类型：`internal/domain/palette`（`Config{ Hotkey, TriggerWord, MatchMode string; CommandTriggers map[string]string }` + 默认值 + 校验函数）。校验规则全文见 spec palette-config「命令面板配置读写 API」，此处不改写。
- Wire DTO：HTTP JSON 使用 camelCase 四键 `hotkey` / `triggerWord` / `matchMode` / `commandTriggers`。GET 与 PUT 200 均返回 `{"hotkey":"mod+k","triggerWord":"new","matchMode":"exact-then-substring","commandTriggers":{...}}` 形状（camelCase 四键；`commandTriggers` 键为指令 ID 枚举 `command-center|projects|settings-appearance|settings-env|settings-opencode|settings-ai|settings-palette|register-project`，恰 8 键，未启用的值为空字符串；默认值 `command-center:'cc'`、`projects:'pro'`、`register-project:'reg'`、其余 5 键为空字符串）；领域结构字段为 Go 导出名，adapter 负责 camelCase wire DTO 映射，领域层不以下划线 JSON tag 对外。
...
- `commandTriggers`（4.9-4.11 前置契约）：`diskConfig`（palette.json）增加 `commandTriggers` 键；磁盘迁移语义（规范源 spec palette-config「命令面板配置存储」）：磁盘文件缺少 `commandTriggers` 键（旧版本写入）时，加载 MUST NOT 降级——按其余三键正常加载并复制默认词表，但与旧 `triggerWord` fold 后相同的默认项 MUST 置为空字符串（保自定义词，冲突指令暂不启用），其余默认项保留；磁盘文件含 `commandTriggers` 但非法（未知 ID/键不全/值非法/重复冲突）时按既有损坏语义降级默认配置并告警。LoadStore 默认词表（`DEFAULT_COMMAND_TRIGGERS`）：`command-center:'cc'`、`projects:'pro'`、`register-project:'reg'`、其余 5 键为空字符串。Go 侧冲突比较（迁移置空与 PUT 去重）的 fold 实现落点：因 domain 层 stdlib-only 架构守卫（D0），`FoldForMatch` 落在 `internal/infrastructure/palette` 并以 `Config.Validate(fold func(string) string)` 注入，全部生产路径（Put/LoadStore/迁移/API 预校验）注入同一实现。 MUST 与 ECMAScript `toLowerCase` 折叠语义兼容（覆盖 U+0130 `İ` 全映射 → `i\u0307` 与 Greek final sigma 上下文规则 `ΟΣ` → `ος`；Go `strings.ToLower` 两者均不满足）：实现固定为 `golang.org/x/text/cases` 的 `cases.Lower(language.Und)`（新增依赖；行为已验证：`İ` → `i\u0307`、`ΟΣ` → `ος`、`ẞ` → `ß`）；MUST NOT 使用 `strings.EqualFold`、裸 `strings.ToLower` 或仅含 `İ` 特判的实现代替。
...
`internal/api/palette_config.go`：`GET /api/v1/palette/config`、`PUT /api/v1/palette/config`（镜像 `internal/api/ai_config.go` / `internal/api/notification_config.go:69` 形态）。API 行为见 spec palette-config「命令面板配置读写 API」：空请求体、纯空白体、JSON 语法错误、尾随第二个 JSON 值均为 400 `invalid_input`；语法合法但顶层非对象（数组/字符串/数字等）、四键缺失/null/类型错误为 422 `invalid_input`；业务非法为 422 `invalid_input`；写盘失败为 500 `internal`；未知附加键按项目惯例忽略（统一错误结构见 `internal/api/errors.go:27`）。请求体上限为 4 KiB：`<=4096` 字节继续解码、`>4096` 字节返回 400；该 400 的错误信封 code 为 `invalid_input`；超限请求 MUST NOT 进入校验或文件写路径。PUT 仅接受规范串，非规范串返回 422。`commandTriggers` PUT 校验矩阵（4.9-4.11 前置契约，规范源 spec palette-config「命令面板配置读写 API」）：PUT 四键全必填（`hotkey`、`triggerWord`、`matchMode`、`commandTriggers`）；`commandTriggers` 缺失、非对象、含未知指令 ID 键、键不全（须恰含 8 键）→ 422 `invalid_input`；非空值沿用 triggerWord 字符规则（非空、禁空白字符集合、≤32 Unicode code point）；非空值之间按 `foldForMatch` 比较不可重复、不可与全局 `triggerWord` 相同（违反 → 422）；值之间前缀重叠允许（解析按最长前缀优先）。Go 侧冲突比较的 fold MUST 与 ECMAScript `toLowerCase` 折叠语义兼容（覆盖 U+0130 `İ` 全映射 → `i\u0307` 与 Greek final sigma 上下文规则 `ΟΣ` → `ος`；Go `strings.ToLower` 两者均不满足）：实现固定为 `golang.org/x/text/cases` 的 `cases.Lower(language.Und)`（新增依赖；行为已验证：`İ` → `i\u0307`、`ΟΣ` → `ος`、`ẞ` → `ß`）；MUST NOT 使用 `strings.EqualFold`、裸 `strings.ToLower` 或仅含 `İ` 特判的实现代替。
...
- 解析与排序
  ...
  - jsdom 组件测试：`CommandPalette` 触发词模式（置顶命令、候选排序、Enter 执行、候选点击携带 projectID、`new zzzz` 无任何命中时展示全部项目候选、空余文展示全部项目候选且 MUST NOT 自动预选、命中时候选默认高亮首位且 Enter 携带 `projectID + projectName`、空余文与零命中时默认高亮置顶命令、余文变化后默认高亮重置）
...
- 指令触发词（4.9-4.11 前置契约）
  - Go domain/store/API：未知指令 ID → 422、键不全 → 422、重复/与全局 `triggerWord` 相同 → 422、磁盘迁移（缺 `commandTriggers` 键不降级；复制默认词表时与旧 `triggerWord` fold 冲突的默认项置空、其余保留；旧 `triggerWord=cc/CC/pro/reg` 迁移用例）、GET 返回全部 8 键、默认词表（`cc`/`pro`/`reg` + 5 空键）落盘；冲突比较 fold 用例：`Ä/ä`（折叠后相同判重复）、`İ/i\u0307`（cases.Lower 后判重复）、`İ/i`（不判重复）、`ΟΣ/ος`（final sigma 后判重复）、`ΟΣ/οσ`（不判重复）
  - 设置面板：8 行渲染、冲突提示不 PUT、draft 含 8 键初始化、默认配置（5 空键）加载后不修改直接保存成功
  - CommandPalette：`cc ` 模式唯一条目默认高亮 Enter 执行、`cc` 余文忽略、最长前缀 `p`/`pr`、仅词无空白不进模式、与 `new` 快速新建模式互不干扰
  - CommandPalette（4.12）：`pr ocdeck` 候选按排序展示+默认高亮首位+候选 Enter/点击导航 `#/projects#<id>`、置顶命令唯一精确/唯一子串（matchMode）推断导航该项目深链、零/多命中导航 `/projects` 不选中、缩写命中不参与推断、空余文/零命中默认高亮置顶命令、`pr` 与 `new` 模式互不干扰回归
- 信号与面板
...
- API/store
  - Go 测试：`internal/infrastructure/palette` store 测试（不存在→默认、损坏→降级、原子写、落盘 JSON 精确键名测试（camelCase 四键，禁止 PascalCase 键）、`os.Stat(path).Mode().Perm()==0600` 断言）与 `internal/api` 校验拒绝测试，`t.TempDir()` 模式（先例：`internal/api/notification_config_test.go`）。
  - 补充矩阵：API 必填/null/422/500/`>4096` 字节 400 `invalid_input`/旧快照不变矩阵、空请求体/纯空白体/JSON 语法错误/尾随第二个 JSON 值为 400 `invalid_input`、语法合法但顶层非对象为 422 `invalid_input`、合法 JSON 补空白至恰好 4096 字节返回 200、4097 字节返回 400 且不进入校验/写盘
- App 生命周期
...
- Lane A：冻结 domain/wire contract 与纯函数（fuzzy-match.ts、hotkey.ts、domain/palette）；事件常量 `od:palette-config-changed` 与 `PaletteConfig`/`PaletteMatchMode`/detail 类型属 Lane A 冻结范围（`type PaletteMatchMode = 'exact' | 'exact-then-substring'`，并用于组件 props 与匹配函数签名）；`MatchKind` / `MatchResult` / `classifyMatch` / `rankByQuery` 签名（D3，含 4.8 增量扩展的 `'acronym'` 档位与 `acronymOf`）属 Lane A 冻结范围；多指令触发词（4.9-4.11 前置契约）：`type PaletteCommandId = 'command-center' | 'projects' | 'settings-appearance' | 'settings-env' | 'settings-opencode' | 'settings-ai' | 'settings-palette' | 'register-project'`、`PaletteConfig` 增加第四键 `commandTriggers: Record<PaletteCommandId, string>`、`DEFAULT_COMMAND_TRIGGERS` 常量（`cc`/`pr`/`reg` + 5 空键）属 Lane A 冻结范围。
