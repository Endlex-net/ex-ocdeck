# mobile-terminal-adaptation spec delta
...
触屏设备解锁状态下，当 xterm 运行时 mouse tracking active 时，终端 SHALL 支持 tap（位移小于阈值、短时长）转换为合成 mousedown/mouseup 事件，经 xterm 鼠标协议路径转为点击序列发送给 TUI，以实现触摸点击审批确认等 TUI 内交互。合成点击 MUST 对原 touch 序列 `preventDefault()` 抑制浏览器 compatibility mouse 事件，保证一次 tap 恰好产生一次点击（防审批双确认）。本需求以 spike 验证 opencode TUI 在真实 web/xterm→WS→tmux 链路开启 mouse reporting 为前提；spike 阴性时本 change 不得交付（升级决策），不得静默降级验收口径。
...
### Requirement: 终端锁定模式
触屏设备（`pointer: coarse`）上终端 SHALL 在每次 WS 连接建立（含一切重连、Tab 切换）后进入锁定状态：透明 overlay 拦截全部 pointer/touch（textarea 不聚焦、虚拟键盘不弹出），`term.blur()` 保证硬件键盘无输入目标。统一输入门禁 MUST 覆盖 xterm 的全部发送出口（`onData` 与 `onBinary`，后者承载鼠标控制序列等非 UTF-8 字节），锁定期除合成手势产生的滚动字节外零 stdin 发送。门禁状态 MUST 先于 `term.blur()` 生效，防止 focus-out 控制序列在锁定瞬间泄漏。用户 SHALL 可显式解锁交互、随时重新锁定。锁定状态 SHALL NOT 持久化。桌面端（fine pointer）SHALL NOT 默认锁定且不显示锁定 UI。pointer 类型动态变化（如 iPad 接/拔外接鼠标或触控板）时系统 MUST 重评估：转入 fine 自动解锁并隐藏锁定 UI，转入 coarse 强制锁定并显示锁定 UI。锁定/解锁 MUST NOT 中断 WS/PTY 数据流或触发重连。

#### Scenario: 触屏设备默认锁定
- **WHEN** 用户在 iPad/手机打开任务终端
- **THEN** 终端锁定，点击终端区域不弹键盘、不产生输入，输出正常渲染

#### Scenario: 重连强制锁定
- **WHEN** 触屏设备终端 WS 断开重连成功（含切换 tab 引起的重连）
...
#### Scenario: 外接鼠标/触控板接入自动解锁
- **WHEN** 锁定中的 iPad 接入外接鼠标/触控板（`pointer` 媒体特性变为 fine）
- **THEN** 终端自动解锁、锁定 UI 隐藏，用户可直接输入；拔出后恢复锁定。注：仅外接键盘（无指针设备）不改变 `pointer`，终端保持锁定，用户可手动解锁——`pointer` 描述主指针设备，无法检测键盘
...
#### Scenario: 桌面端无锁定干扰
- **WHEN** 用户在桌面浏览器（fine pointer）打开终端
- **THEN** 终端直接可交互，不显示锁定/解锁按钮

### Requirement: 虚拟键盘视口适配
系统 SHALL 在解锁且终端聚焦时监听 `visualViewport` 的 resize 与 scroll 事件（Safari 为保持输入点可见会平移 visual viewport），按 `max(0, visualViewport.offsetTop + visualViewport.height - 容器 top)` 调整终端外层容器（承载终端画面、锁定 overlay 与浮动按钮的 wrap）高度并触发 FitAddon 重排与 PTY resize 同步，保证输入区与浮动锁定按钮不被虚拟键盘遮挡；blur/锁定后恢复原高度。visualViewport API 缺失时 SHALL 跳过（可接受遮挡降级）。

#### Scenario: 解锁弹出键盘
- **WHEN** 用户在 iPad/手机解锁终端并弹出虚拟键盘
