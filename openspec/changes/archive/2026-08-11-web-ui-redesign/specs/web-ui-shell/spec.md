...
侧栏任务组数据 MUST 来自 App 层共享 projects 轮询 store（5s single-flight）：壳层侧栏、指挥中心与项目管理页消费同一数据源，应用内 MUST NOT 存在第二个 `/projects` 轮询。store MUST 暴露 `refresh()`（trailing 语义）：任何变更操作成功后调用；若调用时已有轮询在途，MUST 在该请求结束后再补发一次——`refresh()` 承诺其结果反映调用之后的最新状态（MUST NOT 以 mutation 前的在途快照交差）。轮询失败时侧栏 MUST 保留上次成功数据静默展示（错误由 ServerStatusBanner 与页面级错误提示承担，侧栏本身不闪空态）。
