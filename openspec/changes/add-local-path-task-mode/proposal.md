# Proposal: add-local-path-task-mode

## Why

目前 git（repo）项目启动任务只能创建 git worktree，在隔离副本中运行。用户希望直接在已有 repo 的本地目录上就地跑任务（允许 dirty working tree、改动立即生效在原目录），而不想为每个任务新建 worktree。现有 `dir` 项目类型已支持纯目录就地运行，但它是独立项目类型，无法覆盖「已有 repo 项目想偶尔就地跑一个任务」的场景。

## What Changes

- repo（worktree）项目创建任务时，新增任务级运行模式选择：worktree（现状，默认）或 local-path（新增）。
- local-path 模式下任务直接在项目 repo 目录就地运行：不创建 worktree、不创建分支、允许 dirty working tree。
- local-path 任务的 git 语义与现有 dir 项目任务完全一致：无分支、拒绝 base_ref、删除任务时不触碰目录与 git 状态。
- 同一 repo 项目的多个 local-path 任务不限制并发，但 UI 提供低可见度提醒（共享同一工作目录）。
- 任务创建界面新增模式选择器（默认 worktree），选择 local-path 时展示低可见度提示。
- 现有 dir 项目类型保留并存，不收敛、不迁移。

非目标：
- 不改变 worktree 模式的任何现有行为（仍为默认）。
- 不新增项目类型；dir 项目类型语义不变。
- 不支持 local-path 任务的 base_ref / 分支相关能力。

## UI 设计说明与框图

改动位置：指挥中心「新建任务」内联面板（`web/src/pages/CommandCenterPage.tsx`）。核心原则：**用户主动选择的非默认行为用「弱」层级提醒（灰字），不用色块**；色块警告留给系统兜底场景（dir 项目天生无隔离）。

**运行模式选择器**：双段 segmented control（隔离 worktree / 就地运行），插在「项目」与「基准分支」之间，仅 repo 项目渲染，默认「隔离 worktree」（默认行为与现状一致）。

**模式联动**：选「就地运行」时基准分支字段（含刷新远端分支按钮）整体隐藏而非禁用；提交门禁绕过分支列表 ready 等待；切换模式不清空已选分支、不重发请求。

**低可见度提醒**：选「就地运行」时在选择器下方显示 12px 灰字提醒（无色块无边框）：「直接在项目目录里跑，改动就地生效。多任务共享同一目录，并行与否自己把握。」

**dir 项目兼容**：dir 项目不显示模式选择器，现有色块警告原样保留；两类提示条件互斥、天然不叠加。

默认态（repo + 隔离 worktree，仅多一行「运行模式」）：

```
┌──────────────────────────────────────────────────────────┐
│ 新建任务                                                  │
│ 项目                                                      │
│ ┌──────────────────────────────────────────────────────┐ │
│ │ ocdeck                                               │ │
│ └──────────────────────────────────────────────────────┘ │
│ 运行模式                                                  │
│ ┌────────────────────┬───────────────────┐               │
│ │ ▓ 隔离 worktree ▓  │    就地运行        │               │
│ └────────────────────┴───────────────────┘               │
│ 基准分支                                                  │
│ ┌──────────────────────────────────────────────────────┐ │
│ │ main                                                 │ │
│ └──────────────────────────────────────────────────────┘ │
│ 任务名                                                    │
│ ┌──────────────────────────────────────────────────────┐ │
│ │ 例如：重构 agent 通信                                  │ │
│ └──────────────────────────────────────────────────────┘ │
│ 创建后自动切出独立分支与 worktree，并进入工作台；           │
│ 任务名将生成英文分支 slug。                                │
│                              [ 取消 ] [ 创建并进入工作台 ] │
└──────────────────────────────────────────────────────────┘
```

就地运行选中态（基准分支隐藏、灰字提醒、底部 hint 换文案）：

```
┌──────────────────────────────────────────────────────────┐
│ 新建任务                                                  │
│ 项目                                                      │
│ ┌──────────────────────────────────────────────────────┐ │
│ │ ocdeck                                               │ │
│ └──────────────────────────────────────────────────────┘ │
│ 运行模式                                                  │
│ ┌────────────────────┬───────────────────┐               │
│ │   隔离 worktree    │ ▓   就地运行   ▓  │               │
│ └────────────────────┴───────────────────┘               │
│ ⓘ 直接在项目目录里跑，改动就地生效。                      │
│   多任务共享同一目录，并行与否自己把握。                    │
│ 任务名                                                    │
│ ┌──────────────────────────────────────────────────────┐ │
│ │ 例如：重构 agent 通信                                  │ │
│ └──────────────────────────────────────────────────────┘ │
│ 创建后直接在当前目录运行并进入工作台，不切分支。            │
│                              [ 取消 ] [ 创建并进入工作台 ] │
└──────────────────────────────────────────────────────────┘
```

dir 项目态（无「运行模式」选择器，现有色块警告原样保留）：

```
┌──────────────────────────────────────────────────────────┐
│ 新建任务                                                  │
│ 项目                                                      │
│ ┌──────────────────────────────────────────────────────┐ │
│ │ scratch-notes                                        │ │
│ └──────────────────────────────────────────────────────┘ │
│         （无「运行模式」选择器，dir 本就就地运行）           │
│         （无「基准分支」，与现状一致）                      │
│ 任务名                                                    │
│ ┌──────────────────────────────────────────────────────┐ │
│ │                                                      │ │
│ └──────────────────────────────────────────────────────┘ │
│ ┌──────────────────────────────────────────────────────┐ │
│ │ ⚠ 纯目录项目无文件隔离：多个并行任务共享同一目录。       │ │
│ └──────────────────────────────────────────────────────┘ │
│                              [ 取消 ] [ 创建并进入工作台 ] │
└──────────────────────────────────────────────────────────┘
```

两类提示条件互斥：local-path 灰字提醒只在 repo 项目手动选「就地运行」时出现，dir 色块警告只在 dir 项目出现，天然不叠加；灰字 vs 色块的层级差即「有选择 vs 无选择」的语义差。

## Capabilities

### New Capabilities

（无）

### Modified Capabilities

- `task-lifecycle`: repo 项目的任务创建/运行/删除生命周期新增任务级 local-path 模式（就地运行、无 worktree/分支、并发提醒语义）。
- `command-center`: 新建任务面板新增运行模式选择器与 local-path 联动（隐藏基准分支、绕过分支 ready 门禁、灰字提醒）。
- `env-management`: 生命周期变量注入按任务模式分流（local-path 不注入分支变量，同 dir）。
- `project-management`: 项目列表任务摘要字段表增加 `mode`；worktree 存放位置约定限定为 worktree 模式任务。
- `task-detail-stream`: 任务详情 DTO 字段穷举增加 `mode`。
- `git-operations`: 任务级 git 操作（status/diff/commit/push）与 diff review 门禁扩展到 local-path 任务（同 dir 拒绝）。
- `opencode-orchestration`: session 对齐/认领语义按任务有效模式分流（local-path 走 ownedOnly，同 dir）。
- `project-lifecycle-config`: inherit/init/pre-delete 的 kind 语义覆盖 local-path 任务（同 dir：不 inherit、init cwd 为项目目录、pre-delete 用户授权例外）。
- `active-sessions-stream`: 活跃任务流 SSE DTO 增加 `mode` 必有字段（mode 全链路透传的一环）。
- `active-sessions-overview`: 活跃任务概览 REST DTO 增加 `mode` 必有字段（与 SSE 同构）。

## Impact

- 任务数据模型：`tasks.mode` 列（migration 0013），任务级运行模式持久化与回填（存量 repo→worktree、dir→local-path）。
- 任务生命周期分流点（14 处）：创建/重试/删除/对齐/env 注入/git 门禁从项目级 kind 改为任务级有效模式（`internal/task/`）。
- API：`POST /api/v1/projects/{id}/tasks` 增加 `mode` 字段；任务 DTO / 项目任务摘要 / 详情 SSE / 活跃任务概览 REST 与 SSE 增加 `mode` 必有字段。
- Web：新建任务面板模式选择器与低可见度提醒；工作台 Git tab 与删除确认弹窗按任务 mode 降级（`web/src/`）。
- 复用现有 dir 项目任务语义（`createDir` 路径），不引入新的项目类型。
