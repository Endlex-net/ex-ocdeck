...
任务通知能力：将「需要人工介入」的信号从 LLM 触发的插件通知迁移为 ocdeck 进程内事件驱动的确定性通知——五类触发（等待回答问题/等待权限确认/空闲超时/重试未恢复/错误未恢复）× 可配置三渠道（web/Bark/macOS 本地），通知含跳转链接与可选 LLM 停止原因总结（仅 idle），配置全局热更新。
