...
//   - 当前 runtime 令牌 == 触发令牌 → runtime 仍在触发代 → 捕获 attention 可见快照，
//     登记 preCleanup（worker 持锁清理），登记成功后发布 attention 失效 + resync；
//   - runtime 为 nil 且 tombstone == 触发令牌 → cleanup 已在等锁期间完成（attention
...
// runtime 快照指针，不得丢弃存活校验。登记成功才发布（避免 publish-then-reject；
// NoopPublisher 阶段调用位就绪无实际发布）。本方法不持任务锁（等锁已失败）。
func (m *Manager) registerDebtIfCurrent(taskID string, trigger runtime.InstVersion, reason string) {
	...
	if rt := m.getRuntime(taskID); rt != nil {
		...
		current = &cur
		if cur == trigger {
			// 触发令牌仍为当前代：cleanup 尚未发生，捕获外部可见快照（design.md D2
			// 超时路径 (1)→(2)；清理后不可再读）。
...
// 快照存在性（cleanup 后快照已清空，不可事后读取）；worker postCleanup/nil-runtime
// 分支传 false（该令牌的失效已在超时登记或前轮 preCleanup 发布过）。
func (m *Manager) convergeCommitCAS(ctx context.Context, taskID, reason string, tok runtime.InstVersion, attentionVisible bool, cleanupErr error) {
	...
	m.applyConvergeCASMatrix(taskID, tok, attentionVisible, committed.Matched, statusErr)
}
...
//      同令牌债务（若存在）；不发 attention 失效（迁移事件承载任务级失效）；
//   ② CAS error → 保守发布一次 attention 可见失效 + resync.requested（发布先于重读
//      分叉，覆盖 ②a/②b/②c，design.md D2「②CAS error → 保守发布实际已发生的可见失效
//      …然后按状态重读结果分叉」）；重读仍 active 或读错误（②a/②c）→ 登记 postCleanup；
//      重读非 active/缺失（②b）→ compare-and-delete，不登记；
//   ③ !Matched（并发已转走，无错误）→ 重读分叉：③a 仍 active → 发布一次 attention
//      可见失效 + 登记 postCleanup；③b 非 active/缺失 → 不发布，compare-and-delete；
//      ③c 读错误 → 同 ②c（attention 失效 + resync + 登记 postCleanup）。
func (m *Manager) applyConvergeCASMatrix(taskID string, tok runtime.InstVersion, attentionVisible bool, committed bool, statusErr error) {
	switch {
	case statusErr == nil && committed:
		// ① 任务已离开 active。
		m.runtimeRegistry.CompareAndDelete(taskID, tok)
	case statusErr != nil:
		// ② 提交结果不确定：保守发布 attention 可见失效 + resync.requested。
	...
	default:
		...
		switch {
		case !known:
			// ③c 重读错误：保守发布 attention 失效 + 登记 + resync（同 ②c）。
			m.publishAttentionInvalidation(taskID, tok, attentionVisible)
			m.requestResync()
			m.registerConvergeDebt(taskID, tok, runtime.DebtPhasePostCleanup)
		case active:
			// ③a 仍 active（并发窗口）：发布 attention 可见失效 + 登记 postCleanup
			// 由 worker 重试。
			m.publishAttentionInvalidation(taskID, tok, attentionVisible)
			m.registerConvergeDebt(taskID, tok, runtime.DebtPhasePostCleanup)
		default:
...
func (m *Manager) processConvergeDebtLocked(ctx context.Context, entry runtime.DebtEntry) {
	...
	case runtime.DebtPhasePreCleanup:
		if rt := m.getRuntime(entry.TaskID); rt != nil {
			...
			// 清理前捕获 attention 可见快照（worker 持锁执行同一矩阵，design.md D2）；
			// 登记已标记失效发布过（超时路径已发布）则不再计可见，防同一事实二次发布。
			...
			m.advanceDebtAndRunCAS(ctx, entry, attentionVisible, cleanupErr)
			return
		}
		...
		// 该令牌的 attention 失效已在超时登记时发布）→ 推进 postCleanup 仅重试 CAS。
		m.advanceDebtAndRunCAS(ctx, entry, false, nil)
	case runtime.DebtPhasePostCleanup:
		...
		// 仅重试 CAS（env 清 + writeStatusConditional + ①/②/③）。attention 失效已在该
		// 令牌的超时登记或前轮 preCleanup 发布过，本轮不可见（false）。
		m.convergeCommitCAS(ctx, entry.TaskID, convergeDebtPostCleanupReason, entry.Token, false, nil)
	}
}

// advanceDebtAndRunCAS 将债务原子推进 postCleanup 后执行持锁 CAS 收敛（preCleanup 清理
// 完成后调用）。attentionVisible 为清理前捕获的外部可见快照存在性，随矩阵穿透。
// 推进失败：令牌已换（新代）→ 直接退出不删除；记录缺失 → 任务仍 active 且 runtime 已空
// → 重新登记 postCleanup（design.md D2）。
func (m *Manager) advanceDebtAndRunCAS(ctx context.Context, entry runtime.DebtEntry, attentionVisible bool, cleanupErr error) {
	...
	m.convergeCommitCAS(ctx, entry.TaskID, convergeDebtPreCleanupReason, entry.Token, attentionVisible, cleanupErr)
}
