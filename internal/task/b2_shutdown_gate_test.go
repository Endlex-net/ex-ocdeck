...
func TestB2_ShutdownGate_JoinsInFlightAutoActivate(t *testing.T) {
	...
	row, err := m.Create(context.Background(), "p1", CreateTaskOptions{Name: "Gate Task"})
	if err != nil {
...
func TestB2_ShutdownTimeoutOnStuckAutoActivate(t *testing.T) {
	...
	if _, err := m.Create(context.Background(), "p1", CreateTaskOptions{Name: "Stuck Task"}); err != nil {
		t.Fatalf("Create: %v", err)
	}
