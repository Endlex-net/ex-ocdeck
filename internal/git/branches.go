...
// 用 `--format=%(refname:short)%09%(symref)`，仅排除 `%(symref)` 非空的条目（symbolic ref，
// 如 `origin/HEAD`）；真实远端分支 `origin/feature/HEAD`（symref 为空）保留。本地分支逻辑不变
// （本地分支非 symbolic，symref 恒空，不过滤；合法本地分支名 `feature/HEAD` 保留）。
//
...
func ListBranches(ctx context.Context, repoPath string) ([]string, error) {
	localOut, _, err := run(ctx, repoPath, "branch", "--format=%(refname:short)")
	if err != nil {
		return nil, fmt.Errorf("git branch (local): %w", err)
	}
	// 远端分支附带 symref 元数据：%(symref) 非空即为 symbolic ref（origin/HEAD），排除。
	remoteOut, _, err := run(ctx, repoPath, "branch", "-r", "--format=%(refname:short)%09%(symref)")
	if err != nil {
...
// parseLocalBranchLines 按行切分本地 branch --format=%(refname:short) 输出，去空行。
...
// parseRemoteBranchLines 按行切分远端 `branch -r --format=%(refname:short)%09%(symref)` 输出。
